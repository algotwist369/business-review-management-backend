...
const getJsRecords = async (req, res) => {
    try {
        ...
        const data = await JustdialManagementTeam.find(filter)
        ...
        const total = await JustdialManagementTeam.countDocuments(filter);
        ...
        console.error('Get JD Records Error:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};

// Create or update JD record
const createOrUpdateJsRecord = async (req, res) => {
    try {
        const {
            business_id,
            is_live,
        ...
        let record = await JustdialManagementTeam.findOne({ user_id: targetUserId, business_id });
        ...
        if (record) {
            ...
            const populated = await JustdialManagementTeam.findById(record._id)
                .populate('business_id', 'business_name location short_code business_link')
                .populate('user_id', 'username email')
                .lean();
            ...
            const populated = await JustdialManagementTeam.findById(newRecord._id)
                .populate('business_id', 'business_name location short_code business_link')
                .populate('user_id', 'username email')
                .lean();

            return res.status(201).json({
                message: 'JD record created successfully',
                data: populated
            });
        }
    } catch (error) {
        if (error.code === 11000) {
            return res.status(409).json({ error: 'A JD record already exists for this business. Please refresh and try again.' });
        }
        console.error('Create/Update JD Record Error:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};
