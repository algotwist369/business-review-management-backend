...
const getSocialMediaRecords = async (req, res) => {
    try {
        ...
        const data = await SocialMediaManagement.find(filter)
        ...
        const total = await SocialMediaManagement.countDocuments(filter);
        ...
        console.error('Get Social Media Records Error:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};

// Create or update social media record
const createOrUpdateSocialMediaRecord = async (req, res) => {
    try {
        const {
            business_id,
            facebook,
        ...
        let record = await SocialMediaManagement.findOne({ user_id: targetUserId, business_id });
        ...
        if (record) {
            ...
            // Update fields
            if (facebook !== undefined) record.facebook = facebook;
            if (instagram !== undefined) record.instagram = instagram;
            if (twitter !== undefined) record.twitter = twitter;
            if (linkedin !== undefined) record.linkedin = linkedin;
            if (pinterest !== undefined) record.pinterest = pinterest;
            if (status !== undefined) record.status = status;
            if (remarks !== undefined) record.remarks = remarks;
            if (contact_number !== undefined) record.contact_number = contact_number;
            ...
            // Create new record
            const newRecord = await SocialMediaManagement.create({
        ...
        if (error.code === 11000) {
            return res.status(409).json({ error: 'A social media record already exists for this business. Please refresh and try again.' });
        }
        console.error('Create/Update Social Media Record Error:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};
